    ...
    def run(self):
        ...
        while 1:  # endless while
            if Client.countMe:  # if countMe variable in class Client not null
                ...
                print("\nGot message from: " + str(address) + ": " + str(
                    message.decode("utf-8")))  # print message to user
...
class Client(threading.Thread):  # class Client
    ...
    def run(self):
        ...
        print(
            "Now you can Chat with your friend! Let the Coin flipping with: >>coinflip<<. "
            "If you want to close program and the connection type in >>exit<<")  # welcome message
        while 1:  # endless while
            try:
                print(
                    "Send message; make a CoinFlip >>coinflip<<. Close program and the connection >>exit<<")
                # send codewords you can use
                ...
                if msg == 'exit':  # if message like 'exit'
                    ...
                    try:
                        ...
                        hasht = sha512(
                            (str(randomstring)).encode()).hexdigest()  # make sha512 hash from generated string
